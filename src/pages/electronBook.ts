...
export class ElectronBook {
      ...
      <div class="electron-book">
        <div class='container'>
          ...
          <div id="pagination" class="row pagination align-items-center">
            ...
            <input id="pagination-input" type="number" class="pagination__input" min="${
              this.firstPageIdx + 1
            }" max="${this.pageLimitIdx + 1}" value="${this.firstPageIdx + 1}">
  ...
  public initPageNumber(): void {
    const paginationPage = document.getElementById(
      "pagination-page"
    ) as HTMLElement;
  ...
  public switchPage(): void {
    const paginationInput = document.getElementById(
      "pagination-input"
    ) as HTMLInputElement;
    const switchPageBtn = document.getElementById(
      "pagination-switch"
    ) as HTMLElement;
  ...
  public validatePageNumber(
    ...
    if (
      !Number.isNaN(paginationInputIdx) &&
      paginationInputIdx >= this.firstPageIdx &&
      paginationInputIdx <= this.pageLimitIdx
    ) {
  ...
  public initWords(group: number, page: number): void {
    const wordsList = document.getElementById(
      "electron-book-words"
    ) as HTMLElement;
    wordsList.innerHTML =
      '<div class="loader"><div class="lds-ellipsis"><div></div><div></div><div></div><div></div></div></div>';
  ...
  public renderWordsList(): void {
    ...
    if (this.words.length > 0) {
      ...
      wordsList.innerHTML =
        '<div class="electron-book__title title">Список слов пуст</div>';
    }
  }
          ...
          <div class="row word-card">
            ...
            <div class="word-card__body">
              ...
              <div class="word-card__mid">
                ...
                <div class="word-card__meaningtranslate">${
              ...
              <div class="word-card__bottom">
                ...
                <div class="word-card__exampletranslate">${
                  word.textExampleTranslate
                }</div>
              </div>

              <div class="word-card__tracker">
                 <div class="word-card__correctTrack">Верно угадано: ${
                   word.userWord ? word.userWord.optional.totalCorrectCount : 0
                 }</div>
                 <div class="word-card__incorrectTrack">Неверено угадано: ${
                   word.userWord
                     ? word.userWord.optional.totalIncorrectCount
                     : 0
                 }</div>
              </div>
              ...
              ${
                model.auth &&
                model.electronBookGroup !== this.difficultWordsGroup
  ...
  public addDifficultyCardClass(wordCard: HTMLElement, word: IWord): void {
    if (word.userWord && word.userWord.difficulty !== "normal") {
      ...
      const btn = wordCard.querySelector(
        `.btn-${word.userWord.difficulty}`
      ) as HTMLElement;
  ...
  public initAudioPlayerBtn(wordCard: HTMLElement, word: IWord): void {
    Promise.all([
      ...
      const audioBtn = wordCard.querySelector(
        ".word-card__audio"
      ) as HTMLElement;
  ...
  public initCardDifficultyButtons(wordCard: HTMLElement, word: IWord) {
    ...
    hardBtn.onclick = () =>
      this.toggleWordToDifficultWords(wordCard, word, hardBtn, easyBtn);
    easyBtn.onclick = () =>
      this.toggleWordToEasyWords(wordCard, word, hardBtn, easyBtn);
  }

  public toggleWordToDifficultWords(
    ...
    if (!word.userWord) {
      api
        .createUserWord(model.auth!.userId, word._id, {
          difficulty: "hard",
          optional: {
            correctCount: 0,
            totalCorrectCount: 0,
        ...
        .then(() => {
          ...
          (wordCard.firstElementChild as HTMLElement).classList.add(
            "word-card--hard"
          );
      ...
      if (
        (wordCard.firstElementChild as HTMLElement).classList.contains(
          "word-card--hard"
        )
      ) {
        ...
        api
          .updateUserWord(model.auth!.userId, word._id, userWordData)
          .then(() => {
            (wordCard.firstElementChild as HTMLElement).classList.remove(
              "word-card--hard"
            );
  ...
  public initGroups(): void {
    ...
    for (let i = 0; i <= this.groupsCount; i++) {
      ...
      group.classList.add(
        "electron-book__group",
        `electron-book__group--${i + 1}`
      );
  ...
  public initGamesButtons(): void {
    const electronBookAudiocallBtn = document.getElementById(
      "electron-book-audiocall"
    ) as HTMLElement;
    const electronBookSprintBtn = document.getElementById(
      "electron-book-sprint"
    ) as HTMLElement;
    const audioCallBtn = document.getElementById(
      "audio-call-btn"
    ) as HTMLElement;
  ...
  public checkEasyWordsCount(): void {
    ...
    if (
      this.words.every((word) => word.userWord?.difficulty === "easy") ||
      document.querySelectorAll(".word-card--easy").length ===
        this.wordsPerPageLimit
    ) {
  ...
  private createSettingListeners() {
    ...
    header.onclick = (e) => {
      if (
        ((e.target as HTMLElement).id !== "electron-book-btn" &&
          (e.target as HTMLElement).tagName === "LI") ||
        (e.target as HTMLElement).id === "logout-btn"
      ) {
        this.updateSettings();
      }
    };
