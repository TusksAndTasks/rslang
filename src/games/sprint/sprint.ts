import { IWordData, ISprintWord} from "../../types/types";
...
export class Sprint {
  ...
  private sortWords(): Array<IWordData> {
    return [...this.testArray].sort((firstWord, secondWord) => {
      return this.createRandomNumber();
    })
  }

  private createQuestionsArray(): Array<ISprintWord> {
    const questionsArray: Array<number> = new Array(this.testArray.length).fill(0);
  ...
  private setWord(word: HTMLElement, translation: HTMLElement, questionsArray: Array<ISprintWord>, index: number){
